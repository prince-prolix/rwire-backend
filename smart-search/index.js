import { checkSpecialConditions, cleanQuery } from "../common/query-functions.js";
...
export const getElasticQuerySmartSearch =
 async (queryToSearch,options={}) =>{
    const { isNumberWithIncludeSearch = false } = options;
      ...
      if (isValidQuery) {
        const finalSearchQuery = isValidQuery;
        ...
        try {
          if (isNumberWithIncludeSearch) {
            return getElasticQueryNumberSearchWithIncludes(
                processedQuery,
                options
              )
          } else {
  ...
  const getElasticQueryNumberSearchWithIncludes =
